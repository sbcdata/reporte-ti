...
export default function DashboardView() {
  ...
  const estados = useMemo(
    () =>
      Array.from(
        new Set(problemas.map((p) => p.estado).filter(Boolean)),
      ).sort(),
    [problemas],
  );
  ...
  const porCategoria = useMemo(
    () => agruparPor(visiveis, "categoria"),
    [visiveis],
  );
  const porCriticidade = useMemo(
    () => agruparPor(visiveis, "criticidade"),
    [visiveis],
  );
  const porResponsavel = useMemo(
    () => agruparPor(visiveis, "responsavel"),
    [visiveis],
  );
  const porSuperintendencia = useMemo(
    () => agruparPor(visiveis, "superintendencia"),
    [visiveis],
  );

  const criticos = visiveis.filter(
    (p) =>
  ...
  const corCategoria = (_: ContagemItem, i: number) =>
    ...
    <>
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-2 sm:gap-4 lg:grid-cols-4">
          ...
          />
          <KPI
            rotulo="Unidades Monitoradas"
            valor={porUnidade.length}
            accent="#0ea5e9"
            delay={60}
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-5">
          ...
          <Painel
            titulo="Perfil de Criticidade"
            className="lg:col-span-2"
            delay={200}
          >
            <GraficoPizza
              dados={porCriticidade}
              cores={(it) => corCriticidade(it.nome)}
            />
            <div className="mt-4 border-t border-slate-100 dark:border-ink-700/60 pt-4">
              <Legenda
                dados={porCriticidade}
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          ...
          <Painel titulo="Ocorrências por Superintendência" delay={280}>
            <GraficoBarras dados={porSuperintendencia} cores={corSuper} />
          </Painel>
        </div>
      </Shell>
